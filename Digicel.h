...
public:
	...
	bool createCustomerFile(ofstream *fileStream){
		...
		try{
			fileStream = new ofstream("Digicel_Customers",ios::out|ios::app|ios::binary);		
			if(fileStream->fail())
	...
	void saveCardTopUpDetails(){
		...
		writeFileStream.open("Digicel_CardInfomation",ios::app);
	...
	bool findCard(string findCardNumber){
		ifstream	readFileStream;
		string cardStatus;
		readFileStream.open("Digicel_CardInfomation",ios::in);
	...
	bool login(string usrPass){
		const string password = "TheBiggerBetterNetwork2021";
	...
	void viewCustomerBase(){
		...
		try{
			fileStream.open("Digicel_Customers",ios::in);
			if(fileStream.fail()){
